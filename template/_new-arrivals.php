<!--New Arrivals-->
<section id="new-arrivals">
        <div>
          <h2 class="font-mont font-size-xl color-primary">New Arrivals</h2>
          <!--Owl Carousel-->
          <div class="owl-carousel owl-theme">
            <div class="item">
              <div class="product font-roboto">
                <a href="#"
                  ><img src="./assets/products/dress-shirt.jpg" alt="product1"
                /></a>
                <div class="product-info">
                  <h3 class="font-size-lg color-primary">Men's Shirt</h3>
                  <div class="font-size-20 color-primary">
                    <span><i class="fas fa-star"></i></span>
                    <span><i class="fas fa-star"></i></span>
                    <span><i class="fas fa-star"></i></span>
                    <span><i class="fas fa-star"></i></span>
                    <span><i class="far fa-star"></i></span>
                  </div>
                  <div>
                    <span class="font-size-20 color-primary">$49.99</span>
                  </div>
                  <button
                    type="submit"
                    class="font-size-20 product-btn color-primary-bg"
                  >
                    Add to Cart
                  </button>
                </div>
              </div>
            </div>
            <div class="item">
              <div class="product font-roboto">
                <a href="#"
                  ><img src="./assets/products/womens-jeans.jpg" alt="product2"
                /></a>
                <div class="product-info">
                  <h3 class="font-size-lg color-primary">Women's Jeans</h3>
                  <div class="font-size-20 color-primary">
                    <span><i class="fas fa-star"></i></span>
                    <span><i class="fas fa-star"></i></span>
                    <span><i class="fas fa-star"></i></span>
                    <span><i class="fas fa-star"></i></span>
                    <span><i class="fas fa-star"></i></span>
                  </div>
                  <div>
                    <span class="font-size-20 color-primary">$39.99</span>
                  </div>
                  <button
                    type="submit"
                    class="font-size-20 product-btn color-primary-bg"
                  >
                    Add to Cart
                  </button>
                </div>
              </div>
            </div>
            <div class="item">
              <div class="product font-roboto">
                <a href="#"
                  ><img src="./assets/products/womens-dress.jpg" alt="product3"
                /></a>
                <div class="product-info">
                  <h3 class="font-size-lg color-primary">Women's Dress</h3>
                  <div class="font-size-20 color-primary">
                    <span><i class="fas fa-star"></i></span>
                    <span><i class="fas fa-star"></i></span>
                    <span><i class="fas fa-star"></i></span>
                    <span><i class="fas fa-star"></i></span>
                    <span><i class="far fa-star"></i></span>
                  </div>
                  <div>
                    <span class="font-size-20 color-primary">$99.99</span>
                  </div>
                  <button
                    type="submit"
                    class="font-size-20 product-btn color-primary-bg"
                  >
                    Add to Cart
                  </button>
                </div>
              </div>
            </div>
            <div class="item">
              <div class="product font-roboto">
                <a href="#"
                  ><img src="./assets/products/mens-jeans.jpg" alt="product3"
                /></a>
                <div class="product-info">
                  <h3 class="font-size-lg color-primary">Men's Jeans</h3>
                  <div class="font-size-20 color-primary">
                    <span><i class="fas fa-star"></i></span>
                    <span><i class="fas fa-star"></i></span>
                    <span><i class="fas fa-star"></i></span>
                    <span><i class="fas fa-star"></i></span>
                    <span><i class="fas fa-star"></i></span>
                  </div>
                  <div>
                    <span class="font-size-20 color-primary">$49.99</span>
                  </div>
                  <button
                    type="submit"
                    class="font-size-20 product-btn color-primary-bg"
                  >
                    Add to Cart
                  </button>
                </div>
              </div>
            </div>
            <div class="item">
              <div class="product font-roboto">
                <a href="#"
                  ><img src="./assets/products/womens-shoes.jpg" alt="product3"
                /></a>
                <div class="product-info">
                  <h3 class="font-size-lg color-primary">Women's Shoes</h3>
                  <div class="font-size-20 color-primary">
                    <span><i class="fas fa-star"></i></span>
                    <span><i class="fas fa-star"></i></span>
                    <span><i class="fas fa-star"></i></span>
                    <span><i class="fas fa-star"></i></span>
                    <span><i class="far fa-star"></i></span>
                  </div>
                  <div>
                    <span class="font-size-20 color-primary">$89.99</span>
                  </div>
                  <button
                    type="submit"
                    class="font-size-20 product-btn color-primary-bg"
                  >
                    Add to Cart
                  </button>
                </div>
              </div>
            </div>
          </div>
          <!--!Owl Carousel-->
        </div>
      </section>
      <!--!New Arrivals-->